...
//Clock of the execution
int clock =0;

struct queue * enqueue(struct queue * head,int at,int bt,int p,int pi){
...
int main(){
    ...
    int p,a,b,pr;
    for(int i=0;i<n;i++){
    	fscanf(ip,"%d%d%d%d",&p,&a,&b,&pr);
    	jobQ=enqueue(jobQ,a,b,pr,p);
    }    
    fclose(ip);
    sort(&jobQ,n);
    execute(jobQ,n);
