...
struct queue * responseRatio(struct queue * head){
    struct queue * temp = head;
    while(temp!=NULL){
        temp->p->rr = (float) (temp->waTime + temp->p->bTime)/(float)temp->p->bTime;
        temp = temp->next;
    }
    return head;
}

struct queue * updateWaitingTime(struct queue * head){
	struct queue * temp = head;
	while(temp!=NULL){
...
void execute(struct queue * head,int n){
    ...
    readyQ = responseRatio(readyQ);
	schedule(&readyQ,size);
	
	while(readyQ!=NULL || head!=NULL){
		readyQ = updateWaitingTime(readyQ);
		if(readyQ!=NULL){
