...
struct queue * enqueue(struct queue * head,int at,int bt,int pi,pri){
...
void schedule(struct queue ** head,int n){
	...
	for(int i=0;i<n;i++){
		...
		for(int j=0;j<n-1;j++){
			...
			if((q1->p->priority)>(q2->p->priority) && (q1->p->priority)!=(q2->p->priority)){
				*temp = swap(q1,q2);
			}else{
                if((q1->p->prioriy)==(q2->p->priority)){
...
int main(){
    ...
    int p,a,b;
    for(int i=0;i<n;i++){
    	fscanf(ip,"%d%d%d%d",&p,&a,&b,&pri);
    	jobQ=enqueue(jobQ,a,b,p,pri);
    }    
    fclose(ip);
    sort(&jobQ,n);
    execute(jobQ,n);
